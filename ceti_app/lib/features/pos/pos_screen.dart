                  ...
                  if (cart.isNotEmpty)
                    GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            const Icon(Icons.shopping_cart_rounded, color: AppColors.background, size: 16),
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Text('Stock Insuficiente', style: GoogleFonts.plusJakartaSans(color: AppColors.textPrimary, fontSize: 18, fontWeight: FontWeight.bold)),
          ],
        ),
        content: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('No hay suficientes insumos para preparar "${product.name}".', style: GoogleFonts.plusJakartaSans(color: AppColors.textSecondary, fontSize: 14)),
            const SizedBox(height: 12),
            ...reasons.map((r) => Padding(
              padding: const EdgeInsets.only(bottom: 4),
              child: Text('• $r', style: GoogleFonts.plusJakartaSans(color: AppColors.coral, fontSize: 13, fontWeight: FontWeight.w600)),
            )),
          ],
        ),
        actions: [
          TextButton(
            ...
            child: Text('Cerrar', style: GoogleFonts.plusJakartaSans(color: AppColors.textTertiary)),
          ),
          TextButton(
            onPressed: () {
            ...
            child: Text('Autorizar', style: GoogleFonts.plusJakartaSans(color: AppColors.primary, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(
          ...
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('Agregar Producto Libre', style: GoogleFonts.plusJakartaSans(color: AppColors.textPrimary, fontSize: 18, fontWeight: FontWeight.w800)),
            ...
            TextField(
              ...
              style: GoogleFonts.plusJakartaSans(color: AppColors.textPrimary, fontSize: 20, fontWeight: FontWeight.w700),
              decoration: InputDecoration(
                labelText: 'Precio',
                labelStyle: GoogleFonts.plusJakartaSans(color: AppColors.textSecondary, fontSize: 14),
                prefixText: '\$ ',
                prefixStyle: GoogleFonts.plusJakartaSans(color: AppColors.primary, fontSize: 20, fontWeight: FontWeight.w700),
                filled: true,
                fillColor: AppColors.card,
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none),
              ),
          ...
          Column(
            ...
            children: [
              ...
              Text(
                ...
                style: GoogleFonts.plusJakartaSans(
                  color: canSell ? AppColors.primary : AppColors.textTertiary,
                  fontSize: 14,
                  fontWeight: FontWeight.w700,
                ),
              ),
...
class _CheckoutModalState extends State<_CheckoutModal> with TickerProviderStateMixin {
  ...
  void _confirmPayment(BuildContext context, CartProvider cart, double total) {
    if (cart.paymentMethod == PaymentMethod.cash) {
      ...
      if (cashGiven < total) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Monto insuficiente', style: GoogleFonts.plusJakartaSans()), backgroundColor: AppColors.coral),
        );
        return;
      }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('✅ Pago confirmado · ${widget.currency.format(total)}', style: GoogleFonts.plusJakartaSans(color: AppColors.textPrimary)),
        backgroundColor: AppColors.card,
      ...
      builder: (context, ctrl) => Container(
        ...
        child: ListView(
          ...
          children: [
            ...
            Text('Cobrar Pedido', style: GoogleFonts.plusJakartaSans(color: AppColors.textPrimary, fontSize: 20, fontWeight: FontWeight.w800)),
            const SizedBox(height: 4),
            Text('Total: ${widget.currency.format(subtotal)}', style: GoogleFonts.plusJakartaSans(color: AppColors.primary, fontSize: 28, fontWeight: FontWeight.w800)),
            const SizedBox(height: 20),
                  ...
                  children: [
                    Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Text('${item.quantity}', style: GoogleFonts.plusJakartaSans(color: AppColors.textPrimary, fontSize: 14, fontWeight: FontWeight.bold)),
                          IconButton(
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(item.productName, style: GoogleFonts.plusJakartaSans(color: AppColors.textPrimary, fontSize: 14)),
                          if (item.customNotes != null && item.customNotes!.isNotEmpty)
                            Text(item.customNotes!, style: GoogleFonts.plusJakartaSans(color: AppColors.textTertiary, fontSize: 11)),
                        ],
                      ),
                    ),
                    Text(widget.currency.format(item.lineTotal), style: GoogleFonts.plusJakartaSans(color: AppColors.textPrimary, fontSize: 14, fontWeight: FontWeight.w600)),
                  ],
                ),
              );
            }),
            ...
            // ── Efectivo Tab ─────────────────────────────────────────────
            if (cart.paymentMethod == PaymentMethod.cash) ...[
              Text('Monto recibido', style: GoogleFonts.plusJakartaSans(color: AppColors.textSecondary, fontSize: 13)),
                    ...
                    child: Row(
                      children: [
                        ...
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(isSufficient ? 'Cambio a entregar' : 'Faltante', style: GoogleFonts.plusJakartaSans(color: AppColors.textSecondary, fontSize: 12)),
                            Text(
                              isSufficient ? widget.currency.format(change) : widget.currency.format(subtotal - cashGiven),
                              style: GoogleFonts.plusJakartaSans(color: isSufficient ? AppColors.success : AppColors.coral, fontSize: 26, fontWeight: FontWeight.w800),
                            ),
                          ],
                        ),
                      ],
            ...
            if (cart.paymentMethod == PaymentMethod.transfer) ...[
              Center(
                child: Column(
                  children: [
                    Text('Escanea para transferir', style: GoogleFonts.plusJakartaSans(color: AppColors.textSecondary, fontSize: 14)),
                    const SizedBox(height: 16),
                    Container(
                    ...
                    const SizedBox(height: 12),
                    Text(widget.currency.format(subtotal), style: GoogleFonts.plusJakartaSans(color: AppColors.primary, fontSize: 22, fontWeight: FontWeight.w800)),
                    const SizedBox(height: 4),
                    Text('Banco: CETI Pagos · Cuenta: 1234-5678-9012', style: GoogleFonts.plusJakartaSans(color: AppColors.textTertiary, fontSize: 11), textAlign: TextAlign.center),
                  ],
                ),
              ),
            ],
            ...
            if (cart.paymentMethod == PaymentMethod.split) ...[
              Text('Dividir la cuenta', style: GoogleFonts.plusJakartaSans(color: AppColors.textSecondary, fontSize: 13)),
              const SizedBox(height: 16),
              GlassCard(
                padding: const EdgeInsets.all(20),
                borderRadius: 18,
                child: Column(
                  children: [
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        _SplitAmount(label: 'Persona A', amount: subtotal * (_splitPct / 100), currency: widget.currency, color: AppColors.primary),
                    ...
                    Text('${_splitPct.toStringAsFixed(0)}% / ${(100 - _splitPct).toStringAsFixed(0)}%', style: GoogleFonts.plusJakartaSans(color: AppColors.textTertiary, fontSize: 12)),
                  ],
                ),
              ),
            ],
...
class _SplitAmount extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text(label, style: GoogleFonts.plusJakartaSans(color: AppColors.textSecondary, fontSize: 12)),
        Text(currency.format(amount), style: GoogleFonts.plusJakartaSans(color: color, fontSize: 22, fontWeight: FontWeight.w800)),
      ],
    );
  }
}
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        ...
        const SizedBox(width: 4),
        Text(label, style: GoogleFonts.plusJakartaSans(color: AppColors.textTertiary, fontSize: 10)),
      ],
    );
  }
}
